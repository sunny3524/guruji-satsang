...
function DashboardView({user,profile,nav,notify}){
  ...
  useEffect(()=>{
    if(!user) return;
    getSatsangsByOrganizer(user.uid).then(setHosted);
    Promise.all([
      getUserAttendanceSatsangs(user.uid),
      getUpcomingSatsangs(),
    ]).then(([attended, upcoming])=>{
      const attendanceIds = new Set(attended.map(s=>s.id));
      const sevaOnly = upcoming.filter(s=>
        s.organizerUid!==user.uid &&
        Object.values(s.sevas||{}).some(sv=>(sv.enrolled||[]).some(e=>e.uid===user.uid)) &&
        !attendanceIds.has(s.id)
      );
      setAttending([...attended, ...sevaOnly]);
    }).catch(console.error);
  },[user]);
